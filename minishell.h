...
/*   Updated: 2024/10/23 16:45:18 by shebaz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				add_token(Token **tokens, TokenType type,
						const char *value, int *k);
char				*get_executable(char *command);
t_queue				*generate_postfix(t_parser *tokens);
int					get_precedence(int token_type);
void				transfer_tokens_to_stack(t_parser *token_list,
						t_stack **stack);
