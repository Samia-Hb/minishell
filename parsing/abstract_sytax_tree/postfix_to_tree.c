...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/23 13:47:04 by shebaz            #+#    #+#             */
/*   Updated: 2024/10/23 20:27:31 by shebaz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ast	*create_ast_node(t_queue *data, t_ast *node)
{
	...
	new_node->data->token->value = ft_strdup(data->node->value);
	...
	if (node)
	{
		if (data->node->type == TOKEN_REDIR_OUT
			|| data->node->type == TOKEN_REDIR_APPEND)
		{
			new_node->data->output_fd = open(node->data->token->value,
					O_CREAT | O_RDWR, 0777);
		...
		{
			new_node->data->input_fd = open(node->data->token->value, O_RDWR,
					0777);
			if (new_node->data->output_fd == -1)
