...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/23 13:46:46 by shebaz            #+#    #+#             */
/*   Updated: 2024/10/23 13:46:48 by shebaz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_queue	*generate_postfix(t_parser *tokens)
{
	...
	while (head)
	{
		...
		else
		{
			...
			{
				if (is_operator(head->node->token)
					&& is_operator(operator_stack->node->token)
					&& head->node->token->type != TOKEN_CLOSE_PARENTH)
				{
					while (operator_stack && (!check_precedence(operator_stack,
								head->node->token->type)))
					{
				...
				if (head->node->token->type == TOKEN_CLOSE_PARENTH)
				{
					while (operator_stack
						&& operator_stack->node->token->type != TOKEN_OPEN_PARENTH)
					{
						popped = pop_stack(&operator_stack);
						enqueue(&output_queue, &popped);
					}
