...
/*   Updated: 2024/10/23 20:42:15 by shebaz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_consecutive_operator(Token *tokens)
{
	...
	while (current)
	{
		if ((prev->type == TOKEN_PIPE || prev->type == TOKEN_DOUBLE_PIPE
				|| prev->type == TOKEN_DOUBLE_PIPE
				|| prev->type == TOKEN_DOUBLE_AMP
				|| prev->type == TOKEN_REDIR_OUT || prev->type == TOKEN_REDIR_IN
				|| prev->type == TOKEN_REDIR_APPEND
				|| prev->type == TOKEN_REDIR_HERE_DOC)
			&& (current->type == TOKEN_PIPE
				|| current->type == TOKEN_DOUBLE_PIPE
				|| current->type == TOKEN_DOUBLE_AMP
...
int	handle_operators_bg_en(Token *tokens)
{
	...
	if ((is_operator(tokens) && lst_size(tokens) == 1
		...
		|| tokens->type == TOKEN_PIPE || tokens->type == TOKEN_DOUBLE_AMP
		|| tokens->type == TOKEN_DOUBLE_PIPE)
	{
		printf("Syntax Error.\n");
		return (1);
	}
...
{
	if (token_type == TOKEN_REDIR_APPEND || token_type == TOKEN_REDIR_IN
		|| token_type == TOKEN_REDIR_OUT || token_type == TOKEN_REDIR_HERE_DOC)
		return (1);
	return (0);
}
